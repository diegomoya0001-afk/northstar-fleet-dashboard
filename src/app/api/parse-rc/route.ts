...
export async function POST(request: Request) {
  try {
    ...
    const modelsToTry = ['gemini-flash-latest', 'gemini-flash-lite-latest', 'gemini-2.0-flash-lite-001'];
    let lastError = null;
    ...
    for (const modelName of modelsToTry) {
      try {
        ...
        return NextResponse.json({ success: true, data: extractedData, modelUsed: modelName });
      } catch (err: any) {
    ...
    return NextResponse.json({ error: `AI Processing failed. All servers are busy. Last error: ${lastError}` }, { status: 503 });
